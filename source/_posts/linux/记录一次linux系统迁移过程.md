...
拆开电脑，直接加装固态,随便清清灰尘，换一下硅脂(ps: 这不知道是我第几次拆电脑了...,表示以后再也不买GPU风扇了，这已经是第二次失败的购买经历了[``除非得到与原来匹配的风扇一致的风扇，不然我是不再换了``]):
...
进入原先的``linux``系统，对已经安装上的固态硬盘进行分区。因为我之前在没有固态之前，在机械硬盘上就已经安装了三个系统(``win10``, ``manjaro``, ``kali``),因此在分区时，我是考虑只把两个``linux``迁移到固态上，毕竟``win10``现在已经基本不使用了，并将机械硬盘上的``EFI``分区也迁移到固态上。分区这里，可以按照自己的需求对固态进行分区。
...
一般来说，IO调度算法是为低速硬盘准备的，对于固态来说，并不需要使用任何IO调度器。
